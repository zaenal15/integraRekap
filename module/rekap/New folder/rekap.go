...
func SaveKuesioner(w http.ResponseWriter, r *http.Request) {
	...
	isi := r.FormValue("isi_kuesioner")
	table := "kuesioner_rekap"

	result := "failed"
	logInfo := "Submit kuesioner kosong"
	...
	// Ambil session dengan pengecekan nil-safe
	session, err := config.Store.Get(r, config.StoreName)
	if err == nil {
		username, ok1 := session.Values["Username"].(string)
		name, ok2 := session.Values["Name"].(string)
		if ok1 && ok2 {
			model.Log(username, name, "Kuesioner", "submit", logInfo, result)
		}
	}
