...
func LoadSales(w http.ResponseWriter, r *http.Request) {
	...
	id := r.FormValue("id")
	var wherePeriod, whereDrawingImage, whereId, whereDelivery, statusCondition, whereDrawingID string
	...
	if delivery != "" {
		whereDelivery = ` AND status_order IN ('delivery', 'completed')`
	} else {
		whereDelivery = ""
...
`
	...
	COALESCE(a.new_job_number, '-') as new_job_number,
	CONCAT(a.new_job_number, '-', a.reprocess_no) reprocess_id,
...
func LoadSalesReportOverview(w http.ResponseWriter, r *http.Request) {
	...
	queryAmount += ` LEFT JOIN mst_customer x ON z.customer_id = x.customer_id`
	whereCountryReport := ``
	...
	for countryLists.Next() {
		...
		whereCountryReport += `SUM((` + queryAmount + ` WHERE ` + defaultWhere + ` ` + wherePeriod + ` AND z.status_order = 'completed' AND x.country_code = '` + Country_code + `')) as ` + Country_code + `, `
	...
	q := `
		SELECT
		` + whereCountryReport + `
			(` + queryAmount + ` WHERE ` + defaultWhere + ` ` + wherePeriod + ` AND z.status_order = 'completed') as total_month_sales,
			(` + queryAmount + ` WHERE ` + defaultWhere + ` ` + whereFiscalPeriod + ` AND z.status_order = 'completed') as total_period_sales
	`
	fmt.Println(q)
	...
	q = `
	SELECT
		(` + queryAmount + ` WHERE ` + defaultWhere + ` ` + wherePeriod + ` ` + whereFiscalPeriod + ` AND z.status_order != 'cancel' AND z.status_order != 'completed') as leftover_month_sales,
		(` + queryAmount + ` WHERE ` + defaultWhere + ` ` + whereFiscalPeriod + ` AND z.status_order != 'cancel' AND z.status_order != 'completed') as leftover_period_sales
	FROM 
		sales_lists a
		LEFT JOIN mst_customer b ON a.customer_id = b.customer_id 
