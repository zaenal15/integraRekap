...
async function setRekapNilai() {
  ...
  // Mendapatkan nilai filter dari dropdown
  ...
  const optionPeserta = pesertaField ? pesertaField.value : null;
  const optionKategoriLomba = kategoriField ? kategoriField.value : null;
  const optionMataLomba = mataLombaField ? mataLombaField.value : null;

  // Memfilter berdasarkan Peserta jika ada pilihan
  if (optionPeserta && optionPeserta.length > 0) {
    peserta = peserta.filter(peserta => peserta.no_peserta === optionPeserta);
  }

  // Memfilter berdasarkan Mata Lomba jika ada pilihan
  if (optionMataLomba && optionMataLomba.length > 0) {
    mataLomba = mataLomba.filter(mataLomba => mataLomba.lomba_id === Number(optionMataLomba));
    kategoripenilaianLists = kategoripenilaianLists.filter(kategoripenilaianLists => kategoripenilaianLists.lomba_id === Number(optionMataLomba));
  }

  // Memfilter berdasarkan Kategori Lomba jika ada pilihan (commented out in the original code)
  // Uncomment and implement if needed
  if (optionKategoriLomba && optionKategoriLomba.length > 0) {
    kategoriLomba = kategoriLomba.filter(kategoriLomba => kategoriLomba.kategori_id === Number(optionKategoriLomba));
  }
  ...
  Object.values(kategoriMap).forEach(kat => {
    headerKategori += `<th  colspan="${kategoripenilaianLists.length + mataLomba.length}">${kat.nama}</th>`;
    kat.lomba.forEach(lomba => {
      const filteredData = kategoripenilaianLists.filter(item => item.lomba_id === lomba.lomba_id);
      headerLomba += `<th colspan="${filteredData.length + 1}" lomba-id="${lomba.lomba_id}">${lomba.nama_lomba}</th>`;
    });
  });

  headerKategori += `<th rowspan='3'>Sub Total</th><th rowspan='3'>Total Nilai Pinalti</th><th rowspan='3'>Total Akhir</th>`;
  headerLomba += "</tr><tr>";

  mataLomba.forEach(lomba => {
    kategoriFilter = kategoripenilaianLists.filter(item => item.lomba_id === lomba.lomba_id);
    kategoriFilter.forEach(item => {
      headerLomba += `<th lomba-id="${lomba.lomba_id}" class="head-kaegori-penilaian">Total ${item.nama_kategori}</th>`
    });
    headerLomba += `<th class="head-kaegori-penilaian">Nilai Pinalti ${lomba.nama_lomba}</th>`;
  });
  headerLomba += "</tr>";
  ...
  if (peserta.length === 0) {
    ...
    if (tbody) {
      tbody.innerHTML = peserta.map((peserta, idx) => {
        let row = `<tr><td>${idx + 1}</td><td>${peserta.no_peserta}</td><td>${peserta.nama_pangkalan}</td>`;
        ...
        row += `<td class="col-sub-total" peserta-no="${peserta.no_peserta}">0</td><td class="col-nilai-pinalti" peserta-no="${peserta.no_peserta}">0</td><td class="col-total-akhir" peserta-no="${peserta.no_peserta}">0</td>`;
        return row;
      }).join('');
    }
    ...
    peserta.forEach(peserta => {
      peserta.mata_lomba.forEach(lomba => {
        document.querySelectorAll(`.col-kategori-penilaian[lomba-id="${lomba.lomba_id}"][peserta-no="${peserta.no_peserta}"]`).forEach(element => {
          element.classList.replace('tidak-ikut-lomba', 'nilai-belum-diberikan');
...
async function setResultRekapProgress() {
  try {
    // Ambil data nilai rekap dan pengurangan nilai secara bersamaan
    let [rekapData, penguranganData] = await Promise.all([
      fetch('/loadNilaiRekap', { method: 'POST' }).then(res => res.json()).then(data => data.data),
      fetch('/loadPointPenguranganPeserta', { method: 'POST' }).then(res => res.json()).then(data => data.data)
    ]);

    // Cek apakah rekapData dan penguranganData valid, jika tidak, set sebagai array kosong
    // Cek apakah rekapData dan penguranganData valid, jika tidak, set sebagai array kosong
    if (!Array.isArray(rekapData)) {
      rekapData = []; // Tidak melempar error, hanya set jadi array kosong
    }

    if (!Array.isArray(penguranganData)) {
      penguranganData = [];
    }
    // Hitung sub-total nilai juri per peserta
    const pesertaSubTotal = rekapData.reduce((acc, { no_peserta, total_nilai_juri }) => {
      acc[no_peserta] = (acc[no_peserta] || 0) + total_nilai_juri;
      return acc;
    }, {});

    // Hitung total pengurangan nilai per peserta jika ada pengurangan data
    const pesertaPengurangan = penguranganData.reduce((acc, { no_peserta, point_pengurangan }) => {
      acc[no_peserta] = (acc[no_peserta] || 0) + point_pengurangan;
      return acc;
    }, {});

    // Update nilai juri, sub-total, dan total akhir pada elemen HTML
    rekapData.forEach(({ no_peserta, lomba_id, kategori_penilaian_id, total_nilai_juri }) => {
      // Update nilai juri
      const kategoriElement = document.querySelector(
        `.col-kategori-penilaian[lomba-id="${lomba_id}"][kategori-id="${kategori_penilaian_id}"][peserta-no="${no_peserta}"]`
      );
      if (kategoriElement) {
        kategoriElement.textContent = total_nilai_juri;
      }

      // Update sub-total nilai juri
      const subTotalElement = document.querySelector(
        `.col-sub-total[peserta-no="${no_peserta}"]`
      );
      if (subTotalElement) {
        subTotalElement.textContent = pesertaSubTotal[no_peserta] || 0;
      }

      // Hitung total akhir = sub-total - pengurangan
      const totalAkhir = (pesertaSubTotal[no_peserta] || 0) - (pesertaPengurangan[no_peserta] || 0);

      // Update total akhir
      const totalAkhirElement = document.querySelector(
        `.col-total-akhir[peserta-no="${no_peserta}"]`
      );
      if (totalAkhirElement) {
        totalAkhirElement.textContent = totalAkhir;
      }
    });

    // Update nilai pengurangan pada elemen HTML
    penguranganData.forEach(({ no_peserta }) => {
      const totalPenguranganElement = document.querySelector(
        `.col-nilai-pinalti[peserta-no="${no_peserta}"]`
      );
      if (totalPenguranganElement) {
        totalPenguranganElement.textContent = pesertaPengurangan[no_peserta] || 0;
      }
    });

    const totalPointsPinalti = {};

    // Loop melalui data dan hitung total point_pengurangan untuk setiap kombinasi lomba_id dan no_peserta
    penguranganData.forEach(item => {
      const key = `${item.lomba_id}-${item.no_peserta}`;
      if (!totalPointsPinalti[key]) {
        totalPointsPinalti[key] = {
          lomba_id: item.lomba_id,
          no_peserta: item.no_peserta,
          total_point_pengurangan: 0
        };
      }
      totalPointsPinalti[key].total_point_pengurangan += item.point_pengurangan;
    });

    // Update total point pengurangan untuk setiap lomba dan peserta
    for (const key in totalPointsPinalti) {
      const totalPointsPinaltiInfo = totalPointsPinalti[key];
      let boxPointPinalti = document.querySelector(`.col-nilai-pinalti-lomba[lomba-id="${totalPointsPinaltiInfo.lomba_id}"][peserta-no="${totalPointsPinaltiInfo.no_peserta}"]`);
      if (boxPointPinalti) {
        boxPointPinalti.textContent = totalPointsPinaltiInfo.total_point_pengurangan;
      }
    }
...
async function updateTotalNilai(lombaId) {
  try {
    ...
    kategoriData.forEach(kategori => {
      ...
      const filteredData = subPointData.filter(item => item.kategori_penilaian_id === kategori.kategori_id);
      ...
      if (kategoriElement) kategoriElement.textContent = totalNilaiKategori;
    });

    // Update the sub-total value
    document.getElementById('sub-total-nilai').textContent = subTotalNilai || 0;

    const nilaiPinalti = document.getElementById('total-pinalti-nilai').getAttribute('data-value');
    const totalAkhir = subTotalNilai - Number(nilaiPinalti);

    // Update the total-akhir-nilai value, set to 0 if no data
...
async function setKategoriPenguranganPoint() {
  try {
    // Mengambil lombaId dari elemen yang aktif
    const lombaId = document.querySelector('.lomba-header.active').getAttribute('data-lomba-id');
    const formData = new FormData();
    formData.append('lomba_id', lombaId);
    ...
    // Memuat data kategori pengurangan point
    ...
    kategoriData.forEach(value => {
      let contentKriteria = ''; // Inisialisasi content di luar loop

      const dataKriteriaList = kriteriaPenguranganData.filter(item => item.kategori_id === value.kategori_id);

      // Lakukan sesuatu dengan dataKriteriaList
      dataKriteriaList.forEach(item => {
        const itemKriteriaArr = item.kriteria_point_pengurangan.split('-').filter(val => val.trim() !== "");

        // Bangun konten untuk setiap item
        let itemContent = `
      <section value="${item.kriteria_point_pengurangan}" data-id="${item.id}" kategori-id="${item.kategori_id}" class="box-category-content">`;

        // Loop untuk menambahkan tiap elemen dari itemKriteriaArr
        if (itemKriteriaArr.length > 1) {
          itemContent += itemKriteriaArr.map((val, index) => `<span style="display: block;">${index + 1}. ${val}</span>`).join(''); // Menggunakan map dan join untuk optimasi
        } else {
          itemContent += itemKriteriaArr.map((val, index) => `<span style="display: block;"> ${val}</span>`).join(''); // Menggunakan map dan join untuk optimasi
        }
        ...
        `;
        itemContent += `</section>`;
        contentKriteria += itemContent; // Tambahkan konten item ke dalam contentKriteria
      });

      // Update konten pada DOM setelah selesai memproses semua item
...
async function setNilaiPengurangan() {
  try {
    // Retrieve lombaId and noPeserta in a single statement
    ...
    // Initialize total penalty calculation
    let totalPinalti = 0;

    // Ensure data is available and handle the total calculation
    if (Array.isArray(data)) {
      document.querySelectorAll('.total-pengurangan-kategori').forEach(element => {
        const dataId = element.getAttribute('data-id');
        const resultPoint = data.filter(item => item.kategori_id === dataId);
...
async function showRekapJuaraTb(el, KategoriJuaraId) {
  try {
    ...
    console.log('lombaData', lombaData)
...
async function setRekapJuaraPrint(dataLomba, kategorPenilaian, kategoriJuaraId) {
  ...
  console.log('kategorPenilaian', kategorPenilaian)
  // 2. Generate Header Tabel
...
async function setNilaiRekapJuaraPrint(kategoripenilaianLists, dataLomba, kategorPenilaianJuara, kategoriJuaraId) {
  try {
    ...
    // Menampilkan hasil akhir dengan lomba_id
    console.log('kategoriJuaraId', kategoriJuaraId)
    ...
    if (sortedRekapaJuaraPeserta.length === 0) {
      ...
      for (const details of sortedRekapaJuaraPeserta) {
        ...
        no++;
        const noPeserta = details.no_peserta;
        const namaPangkalan = peserta.find(item => item.no_peserta === noPeserta)?.nama_pangkalan || '';
...
async function loadPrintRekapJuara() {
  try {
    ...
    const [kategoriRes, rekapRes, penilaianRes] = await Promise.all([
    ...
    const kategoriList = (await kategoriRes.json()).data || [];
    const rekapList = (await rekapRes.json()).data || [];
    const penilaianList = (await penilaianRes.json()).data || [];

    // Kelompokkan kategori penilaian berdasarkan nama lomba
    ...
    // Ambil data mata lomba tambahan
    const { mataLomba } = await getData();

    // Jika ada kategori juara, proses tiap kategori
    for (const category of kategoriList) {
      const formData = new FormData();
      ...
      if (!response.ok) {
        throw new Error('Gagal memuat data kategori penilaian juara.');
      }

      const result = await response.json();
      const data = result.data || [];

      // Ambil data lomba unik berdasarkan nama_lomba
      const lombaData = Array.from(
        new Map(data.map(item => [item.nama_lomba, {
          lomba_id: item.lomba_id,
          nama_lomba: item.nama_lomba
        }])).values()
      );
      ...
      document.querySelector('#wrap-content-rekap-juara-lomba-print').innerHTML += `
      ...
      `
      // Tampilkan rekap
      kategoriJuaraId = category.id
      await setRekapJuaraPrint(lombaData, data, kategoriJuaraId);
      openModal('#modal-content-report-juara-print')
    }
